...
def train(model, device, train_loader, val_loader, loss_fn, optimizer, epochs, load_model=False):
    ...
    model.to(device)
    best_score = np.inf #tracking our training
    scaler = torch.cuda.amp.GradScaler() #speed of training
    ...
    for epoch in range(epochs):
        ...
        for i, (img, mask) in enumerate(loop):
            ...
            for p in model.parameters(): #zero out the gradients
                p.grad = None
            ...
            loop.set_postfix(loss=loss.item()) #add some training descriptions
...
def visualize(directory, model, transform, device):
    ...
    with torch.no_grad():
        ...
        count = 0
        while success: #as long as video is not finised
            img = transform(image) #transform frame at that point in our video
            img = img.to(device, dtype=torch.float32).unsqueeze(0)
            pred = model(img).argmax(1)
            cv2.imshow(img, pred, count)
            success, image = video.read()
            count += 1
